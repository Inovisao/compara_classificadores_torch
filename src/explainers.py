    ...
    for imgs, labels, filenames in test_dataloader:
        ...
        for i, img in enumerate(imgs):
            ...
            for c in range(len(class_list)):
                ...
                # Plot the image
                axs[c+1].imshow(original_img)
                axs[c+1].imshow(img_for_plot, alpha=0.4)
...
def generate_occlusion(model, model_name, test_dataloader, class_list, device):
    ...
    for imgs, labels, filenames in test_dataloader:
        ...
        for i, img in enumerate(imgs):
            ...
            # Get the attributions for each class and plot them
            for c in range(len(class_list)):
                attributions = explainer.attribute(img.unsqueeze(0), sliding_window_shapes=(3, 16, 16), strides=16, target=c)
                attributions = attributions.cpu().detach().numpy().transpose(0, 2, 3, 1).squeeze()
                ...
                # Resize and normalize (zero if max == min)
                pos_attr = cv2.resize(pos_attr, (image_size, image_size), interpolation=cv2.INTER_NEAREST)
                if pos_attr.max() != pos_attr.min():
                    pos_attr = (pos_attr - pos_attr.min()) / (pos_attr.max() - pos_attr.min())
                else:
                    pos_attr = np.zeros_like(pos_attr)

                neg_attr = cv2.resize(neg_attr, (image_size, image_size), interpolation=cv2.INTER_NEAREST)
                if neg_attr.max() != neg_attr.min():
                    neg_attr = (neg_attr - neg_attr.min()) / (neg_attr.max() - neg_attr.min())
                else:
                    neg_attr = np.zeros_like(neg_attr)
                ...
                # Create rgb image
                pos_attr = np.mean(pos_attr, axis=2)
                neg_attr = np.mean(neg_attr, axis=2)
                img_for_plot = np.zeros((image_size, image_size, 3))
                img_for_plot[:, :, 1] = pos_attr
                img_for_plot[:, :, 0] = np.abs(neg_attr)
                ...
                axs[c+1].imshow(original_img)
                axs[c+1].imshow(img_for_plot, alpha=0.4)
                #axs[c+1].imshow(pos_attr, alpha=0.4, cmap='Greens')
                #axs[c+1].imshow(neg_attr, alpha=0.2, cmap='Reds')
                axs[c+1].set_title(class_list[c], fontdict={'fontsize': 10})
                axs[c+1].axis('off')
            
            fig.tight_layout()
            fig.suptitle(f"Occlusion: label = {class_list[labels[i]]}, prediction = {class_list[pred_indices[i]]}\n", fontsize=10)
            plt.savefig(f"../results/occlusion/{model_name}/is_{class_list[labels[i]]}_pred_as_{class_list[pred_indices[i]]}_{filenames[i]}.png", bbox_inches='tight', dpi=300)
            plt.close()
