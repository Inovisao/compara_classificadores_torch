...
for ((i=1;i<=$ndobras;i+=1)); do
    weights_name="1_resnet18_sgd_0.001.pth"
    folds_name="fold_${i}"
    mkdir -p ./results_gradcam_multilayer_resnet18/${folds_name}
    ...
    python3 resnet_explainer.py -f ${i} -w ${weights_path}/${weights_name} -td ${dobras_dir}/${folds_name} -o ./results_gradcam_multilayer_resnet18/${folds_name}
